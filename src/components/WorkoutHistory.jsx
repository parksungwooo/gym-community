...
function WorkoutMark({ type }) {
  return (
    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-emerald-700 text-[11px] font-black text-white shadow-sm">
      {getWorkoutMark(type)}
    </span>
  )
}
...
function SmallButton({ children, tone = 'default', ...props }) {
  ...
  return (
    <button
      type="button"
      className={`min-h-10 rounded-lg px-3 text-xs font-black transition disabled:cursor-not-allowed disabled:opacity-50 ${toneClass}`}
      {...props}
    >
      {children}
    </button>
...
function PhotoGrid({ items, isEnglish, onOpen, onRemove, onMove, editable = false }) {
  ...
  return (
    <div className={`grid gap-2 ${items.length > 1 ? 'grid-cols-2' : ''}`}>
      {items.map((item, index) => (
        <article key={item.id} className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-white/10 dark:bg-neutral-950">
          ...
          {editable ? (
            <div className="grid gap-2 border-t border-gray-100 p-2.5 dark:border-white/10">
...
function HistoryItem({ item, onUpdate, onDelete, loading, onOpenImage }) {
  ...
  if (editing) {
    return (
      <form className="grid gap-3 rounded-2xl border border-gray-100 bg-gray-50 p-3 dark:border-white/10 dark:bg-neutral-950" onSubmit={handleSave}>
        <div className="flex items-start justify-between gap-3">
          <div className="grid gap-1">
            <span className="text-xs font-black uppercase text-emerald-800 dark:text-emerald-200">{isEnglish ? 'Edit' : '수정'}</span>
            <strong className="text-base font-black leading-6 text-gray-950 dark:text-white">{isEnglish ? 'Edit Workout' : '운동 기록 수정'}</strong>
        ...
        <div className="grid gap-2.5 sm:grid-cols-2">
          <label className="grid gap-2 text-sm font-black text-gray-950 dark:text-white">
            ...
            <select
              className="min-h-10 rounded-lg border border-gray-200 bg-white px-3 text-sm font-bold text-gray-950 outline-none transition focus:border-emerald-500 dark:border-white/10 dark:bg-neutral-900 dark:text-white"
              value={workoutType}
          ...
          <label className="grid gap-2 text-sm font-black text-gray-950 dark:text-white sm:col-span-2">
            {isEnglish ? 'Note' : '메모'}
            <textarea
              className="min-h-20 resize-none rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-semibold leading-6 text-gray-950 outline-none transition placeholder:text-gray-600 focus:border-emerald-500 dark:border-white/10 dark:bg-neutral-900 dark:text-white dark:placeholder:text-gray-300"
              rows="3"
              maxLength="120"
              value={note}
        ...
        <section className="grid gap-3">
          ...
          <div className="grid grid-cols-[1fr_1fr_auto] gap-2">
            ...
            <span className="grid min-h-10 place-items-center rounded-lg bg-gray-100 px-3 text-xs font-black text-gray-700 dark:bg-white/10 dark:text-gray-100">
        ...
        <div className="grid gap-3 border-t border-gray-100 pt-3 dark:border-white/10 sm:grid-cols-[1fr_auto] sm:items-center">
          <div className="grid gap-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button type="submit" className="min-h-10 rounded-lg bg-emerald-700 px-3 text-xs font-black text-white shadow-sm transition hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-50" disabled={loading}>
              {isEnglish ? 'Save' : '저장'}
            </button>
          </div>
        </div>
  ...
  return (
    <article className="grid gap-3 rounded-2xl border border-gray-100 bg-gray-50 p-3 dark:border-white/10 dark:bg-neutral-950">
      <header className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-start">
        <div className="flex min-w-0 items-start gap-2">
          <WorkoutMark type={item.workout_type} />
          <div className="min-w-0">
            <strong className="block truncate text-base font-black leading-6 text-gray-950 dark:text-white">{getWorkoutTypeLabel(item.workout_type, language)}</strong>
        ...
        <div className="flex flex-wrap gap-1.5 sm:justify-end">
          {metaItems.map((meta) => (
            <span key={meta} className="rounded-lg bg-white px-2.5 py-1 text-[11px] font-black text-gray-800 shadow-sm dark:bg-white/10 dark:text-gray-100">{meta}</span>
          ))}
        </div>
      </header>
      ...
      {item.note ? <p className="m-0 rounded-xl bg-white p-2.5 text-sm font-semibold leading-6 text-gray-800 dark:bg-white/10 dark:text-gray-100">{item.note}</p> : null}
    ...
    <section className="grid gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-neutral-900 sm:p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="grid gap-1">
          ...
          <h2 className="m-0 text-xl font-black leading-tight text-gray-950 dark:text-white">{isEnglish ? 'History' : '기록'}</h2>
        </div>
      ...
      {(loading || history.length > 0) && (
        <div className="grid grid-cols-7 gap-1">
          {recentWeek.map((day) => (
            <article key={day.key} className={`grid min-h-12 place-items-center rounded-lg border p-1.5 text-center ${day.done ? 'border-emerald-700 bg-emerald-700 text-white shadow-sm' : 'border-gray-100 bg-gray-50 text-gray-800 dark:border-white/10 dark:bg-neutral-950 dark:text-gray-100'}`}>
      ...
      {openImageUrl ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-gray-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" onClick={() => setOpenImageUrl('')}>
          <div className="grid max-h-[86dvh] w-full max-w-2xl gap-3 overflow-hidden rounded-2xl bg-white p-3 shadow-2xl dark:bg-neutral-900" onClick={(event) => event.stopPropagation()}>
            ...
            <img className="max-h-[72dvh] w-full rounded-xl object-contain" src={openImageUrl} alt={isEnglish ? 'Expanded workout image' : '확대된 운동 이미지'} />
          </div>
        </div>
      ) : null}
    </section>
